    ...
    <div className="max-w-6xl mx-auto px-4 py-4 space-y-4">
      <GlassCard className="p-4 space-y-2">
        ...
        <p className="text-sm text-slate-400">
          Deze matrix helpt je taken te ordenen op urgentie en belang. Richt je
          vooral op <strong>Q II</strong>: belangrijk maar niet urgent werk.
        </p>
      </GlassCard>
      ...
      <GlassCard className="flex flex-wrap items-center gap-2 p-2">
        <select
          ...
          onChange={(e) =>
            setHabit(
              e.target.value === 'all' ? 'all' : (Number(e.target.value) as HabitId)
            )
          }
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        />
        <QuadrantCol
          title="Q II — Niet urgent & Belangrijk"
          tasks={grouped.II}
          onToggle={toggleDone}
          onQuickMove={quickMove}
          onAdjust={adjust}
        />
        <QuadrantCol
          title="Q III — Urgent & Niet belangrijk"
          tasks={grouped.III}
          onToggle={toggleDone}
          onQuickMove={quickMove}
          onAdjust={adjust}
        />
        <QuadrantCol
          title="Q IV — Niet urgent & Niet belangrijk"
          tasks={grouped.IV}
          onToggle={toggleDone}
          onQuickMove={quickMove}
          onAdjust={adjust}
...
function QuadrantCol(
  ...
  return (
    <GlassCard className="p-3 min-h-[40vh]">
      ...
      <div className="space-y-2">
            ...
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex gap-1 flex-wrap">
                {(['I', 'II', 'III', 'IV'] as Quadrant[]).map((q) => (
                  <button
                    key={q}
                    onClick={() => onQuickMove(t, q)}
                    className="px-2 py-1 text-xs rounded-lg bg-white/10 border border-white/10 hover:bg-white/20"
                  >
                    → {q}
                  </button>
                ))}
        ...
        {tasks.length === 0 && (
          <div className="text-sm text-slate-400">Geen taken in dit kwadrant…</div>
        )}
      </div>
    </GlassCard>
  );
